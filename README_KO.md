...
로컬 오디오·영상 또는 Windows PC 재생음을 분석해 Fender Tone Master Pro용 톤 레시피 3개를 추천하는 비공식 Windows 프리뷰입니다. v0.0.05는 선택한 Windows 오디오 입력 또는 PC 재생음 loopback의 파형·주파수·레벨을 확인하는 실시간 스펙트럼 탭을 추가합니다. 릴리스·빌드 기준일은 2026-09-05 KST입니다. v0.0.04에서 추가한 AI 분리의 자동/CPU/CUDA 선택과 성능 진단, NumPy 네이티브 최적화, 출력 연결별 Amp/Cab 체인 정정은 그대로 유지합니다.
...
1. `ToneMatchTMP-v0.0.05.exe`를 실행합니다. 서명되지 않은 프리뷰라 Windows가 경고하면 배포자가 제공한 SHA-256과 파일 해시를 먼저 비교하세요.
...
2. SHA-256을 확인하고 `ToneMatchTMP-v0.0.05.exe`를 실행합니다.
...
2. 압축 안의 `ToneMatchTMP-v0.0.05\source`가 자체 완결된 프로젝트 폴더입니다.
...
오른쪽 위 `개발자 옵션`을 켜면 처리 시퀀스, 실시간 로그, 클릭형 배포 소스와 변경 기록을 확인할 수 있습니다. v0.0.05에서는 `spectrum.py`의 채널별 FFT와 평활화, 장치 모니터와 UI bounded 큐 경로도 배포 소스에서 확인할 수 있습니다. v0.0.04에서 추가한 성능·DSP 진단의 연산 장치, CUDA/GPU/VRAM 상태와 마지막 분리 벤치마크도 유지됩니다. NumPy 최적화는 기존 Python UI·AI 구조를 유지하면서 컴파일된 네이티브 연산을 활용합니다.
...
- C++ 실시간 오디오·링 버퍼·FFT·ASIO 엔진은 후속 로드맵입니다. v0.0.05의 실시간 스펙트럼은 Python 기반 원시 입력 시각화일 뿐, 실시간 입력 톤 매칭이나 C++/ASIO 오디오 엔진이 아닙니다.
...
정식 출시 전 패치는 `0.0.01`, `0.0.02`, `0.0.03`, `0.0.04`, `0.0.05` 순으로 올립니다. 앱 버전, Windows 파일 정보, EXE·ZIP 이름, README, 변경 기록과 SHA-256은 같은 버전이어야 합니다. 다음 패치는 `0.0.06`입니다.
