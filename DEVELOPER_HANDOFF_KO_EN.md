# ToneMatch TMP v0.0.03 — 개발 인수인계 / Developer Handoff

이 문서는 v0.0.03 포터블 개발 ZIP을 다른 Windows PC나 새 개발 세션에서 이어서 작업하기 위한 기준 문서입니다. 대화 기록이 아니라 압축에 포함된 소스와 `BUILD_HISTORY.md`가 작업 상태의 근거입니다.

This is the source-of-truth handoff for continuing the v0.0.03 portable development snapshot on another Windows PC or in a new development session. The archived source and `BUILD_HISTORY.md`, not prior chat history, define the state being handed over.

> v0.0.03 포터블 개발 프리뷰는 빌드·자가진단·manifest 검증을 마쳤습니다. 정확한 산출물 해시는 ZIP과 함께 제공된 `ToneMatchTMP-v0.0.03-SHA256SUMS.txt`를 기준으로 합니다.
> The v0.0.03 portable developer preview completed its build, self-test, and manifest verification. Use the sibling `ToneMatchTMP-v0.0.03-SHA256SUMS.txt` as the authority for artifact hashes.
...
v0.0.03의 보이싱 결과는 톤 레시피와 별도의 참고 진단입니다. 완전한 다성음 채보, 타브 생성 또는 연주 정확도 판정으로 표현하지 않습니다.
...
[FFmpeg: 선택 구간, 최대 20분]
          ↓
[풀믹스이면 Demucs htdemucs_6s → 30초 외부 조각별 guitar stem]
          ↓
[22.05 kHz 분석 PCM → NumPy DSP 특징 + 코드/보이싱 단서(실험)]
          ↓
[픽업·출력 보정 → 18개 템플릿 매칭 → TMP 레시피 3개]
          ↓
[GUI / JSON / HTML / 텍스트]
```
...
| 파일 | 책임 |
|---|---|
| `app.py` | Tkinter UI, 언어/장치/입력 선택, 녹음·분석 작업 스레드, 취소, 내보내기, 자체 진단 |
| `engine.py` | 최대 20분 FFmpeg 디코딩, DSP 특징, AI 분리 호출, 템플릿 매칭, 결과 구조 |
| `separator.py` | Demucs/PyTorch CPU 런타임, 모델 캐시 진단, 30초 조각별 guitar stem 생성 |
| `recorder.py` | SoundCard 기반 WASAPI loopback·오디오 입력 목록과 PCM16 녹음 |
| `voicing.py` | NumPy 기반 피치 클래스·코드 후보·저음/음역 단서 추정(실험, 톤 매칭과 분리) |
...
`DEVELOPMENT_KO.md`는 v0.0.03 구현 구조 요약이고, `FUNCTION_REFERENCE_KO.md`는 배포 소스의 모든 함수와 한국어 docstring을 자동 색인합니다. 새 PC 재구성과 릴리스 판정은 이 문서를 우선합니다.
...
```text
C:\ToneMatchTMP-dev\ToneMatchTMP-v0.0.03\
├─ .venv\                 새 PC에서 다시 생성
├─ ToneMatchTMP-v0.0.03.exe
...
10. EXE `--self-test-output` 결과의 `ok: true` 확인
...
- 전체 `*.py` 소스와 `tests`
- `resources\ffmpeg.exe`
- `ToneMatchTMP.spec`, `version_info.txt`, `build.ps1`, `requirements.txt`
...
검증된 파일명은 `ToneMatchTMP-v0.0.03-Windows-x64-Portable-Dev.zip`입니다. EXE·ZIP·자체 진단의 최종 SHA-256은 형제 파일 `ToneMatchTMP-v0.0.03-SHA256SUMS.txt`에 기록합니다.

### 9. 릴리스 게이트

v0.0.03에서 완료한 항목과 다음 개발자가 재확인할 항목입니다. 실제 loopback 녹음과 코드 서명은 아직 수행하지 않았습니다.

- `catalog.APP_VERSION`, 최신 `CHANGELOG`, `version_info.txt`, spec의 EXE 이름과 모든 문서가 `0.0.03`인지
...
The v0.0.03 voicing output is a separate diagnostic aid. Do not present it as complete polyphonic transcription, tablature, or performance grading.
...
FFmpeg selected range, capped at 20 minutes
       ↓
Demucs htdemucs_6s for full mixes → guitar stem in outer 30-second chunks
       ↓
22.05 kHz analysis PCM → NumPy DSP features + experimental chord/voicing cues
       ↓
pickup/output correction → 18-template match → top three TMP recipes
       ↓
...
Before release, manually exercise guitar-only bypass, first-download full-mix isolation, cached offline isolation, WASAPI loopback recording, 20-minute/end-zero handling, cancellation, both languages, unsupported-device behavior, JSON/HTML export, experimental chord/voicing confidence behavior, and the packaged EXE self-test.

The verified archive name is `ToneMatchTMP-v0.0.03-Windows-x64-Portable-Dev.zip`. Its `source` folder is self-contained and includes source, tests, FFmpeg, build metadata, requirements, documentation, and licenses alongside the EXE built from the same code. Final artifact hashes are in the sibling checksum file. Exclude virtual environments, build caches, model weights, personal media/results, raw logs, credentials, and machine-specific settings.
...
4. Confirm every version-bearing file says `0.0.03` and that `BUILD_HISTORY.md` keeps v0.0.02 and v0.0.03 changes separate.
...
8. Append test, self-test, artifact size, and SHA-256 results to `BUILD_HISTORY.md`.
9. If using a new Codex/task session, explicitly provide the extracted `source` folder (or clone `https://github.com/JH-Prime/ToneMatchTMP`) and ask it to treat these handoff documents as context. Chat history and interrupted analysis state are not embedded in the ZIP.
