...
TEXT: dict[str, dict[str, str]] = {
    ...
    "ui.spectrum_tab": {"ko": "실시간 스펙트럼", "en": "Live spectrum"},
    "ui.spectrum_device": {"ko": "모니터 입력", "en": "Monitor input"},
    "ui.start_spectrum": {"ko": "스펙트럼 시작", "en": "Start spectrum"},
    "ui.stop_spectrum": {"ko": "스펙트럼 중지", "en": "Stop spectrum"},
    "ui.waveform": {"ko": "입력 파형", "en": "Input waveform"},
    "ui.frequency_spectrum": {"ko": "주파수 스펙트럼 · 20 Hz~20 kHz", "en": "Frequency spectrum · 20 Hz~20 kHz"},
    "ui.reference_compare_tab": {"ko": "레퍼런스 비교", "en": "Reference compare"},
    "ui.reference_file": {"ko": "분석 기준", "en": "Analyzed reference"},
    "ui.start_reference_compare": {"ko": "비교 시작", "en": "Start compare"},
    "ui.stop_reference_compare": {"ko": "비교 중지", "en": "Stop compare"},
    "ui.reference_difference": {"ko": "주파수 차이 · 현재 − 기준 (dB)", "en": "Frequency difference · Current − Reference (dB)"},
    "ui.reference_compare_note": {"ko": "레벨 정규화된 음색 차이입니다. +는 현재 입력이 더 강하고 −는 더 약합니다. 일치 확률이나 자동 EQ 추천이 아닙니다.", "en": "This is a level-normalized tonal difference. + means the live input is stronger and − means it is weaker. It is not a match probability or automatic EQ recommendation."},
    "ui.compare_band": {"ko": "대역", "en": "Band"},
    "ui.compare_reference": {"ko": "기준", "en": "Reference"},
    "ui.compare_current": {"ko": "현재", "en": "Current"},
    "ui.compare_delta": {"ko": "Δ 현재−기준", "en": "Δ Current−Reference"},
    "band.low": {"ko": "저역", "en": "Low"},
    "band.low_mid": {"ko": "중저역", "en": "Low Mid"},
    "band.mid": {"ko": "중역", "en": "Mid"},
    "band.presence": {"ko": "프레즌스", "en": "Presence"},
    "band.treble": {"ko": "고역", "en": "Treble"},
    "band.air": {"ko": "에어", "en": "Air"},
    ...
    "status.spectrum_ready": {"ko": "입력 장치를 고른 뒤 시작하세요. 약 20 Hz로 갱신하는 시각 분석기이며 ASIO 저지연 엔진은 아닙니다.", "en": "Choose an input and start. This visual analyzer refreshes at about 20 Hz; it is not an ASIO low-latency engine."},
    "status.spectrum_running": {"ko": "실시간 입력 분석 중 · 약 {rate:.1f} Hz · 중지할 때까지 최신 프레임만 유지합니다.", "en": "Analyzing live input · about {rate:.1f} Hz · only the latest frame is retained until stopped."},
    "status.spectrum_stopping": {"ko": "현재 오디오 블록이 끝나는 대로 스펙트럼을 중지합니다…", "en": "Stopping the spectrum after the current audio block…"},
    "status.spectrum_stopped": {"ko": "스펙트럼을 중지했습니다. 마지막 측정 화면을 유지합니다.", "en": "Spectrum stopped. The last measured frame remains visible."},
    "status.spectrum_failed": {"ko": "실시간 스펙트럼 실패 · 입력 장치와 DSP 상태를 확인해 주세요.", "en": "Live spectrum failed · Check input-device and DSP status."},
    "status.spectrum_backend_pending": {"ko": "DSP · 시작 시 선택", "en": "DSP · selected on start"},
    "status.spectrum_backend_cpp": {"ko": "DSP · C++ PCM/FFT", "en": "DSP · C++ PCM/FFT"},
    "status.spectrum_backend_numpy_missing": {"ko": "DSP · NumPy 대체 · C++ DLL 없음", "en": "DSP · NumPy fallback · C++ DLL missing"},
    "status.spectrum_backend_numpy_abi": {"ko": "DSP · NumPy 대체 · ABI 불일치", "en": "DSP · NumPy fallback · ABI mismatch"},
    "status.spectrum_backend_numpy_unavailable": {"ko": "DSP · NumPy 대체 · C++ 초기화 불가", "en": "DSP · NumPy fallback · C++ unavailable"},
    "status.reference_missing": {"ko": "먼저 로컬 또는 녹음 파일을 분석해 기준 스펙트럼을 만드세요. 참고 URL은 음원 입력이 아닙니다.", "en": "Analyze a local or recorded file first to create a reference spectrum. The reference URL is not an audio input."},
    "status.reference_ready": {"ko": "기준 준비됨 · {file} · 입력 장치를 고른 뒤 비교를 시작하세요.", "en": "Reference ready · {file} · choose an input and start comparing."},
    "status.reference_running": {"ko": "{file} 기준으로 실시간 비교 중 · 약 {rate:.1f} Hz", "en": "Comparing live input against {file} · about {rate:.1f} Hz"},
    "status.reference_stopping": {"ko": "현재 오디오 블록이 끝나는 대로 비교를 중지합니다…", "en": "Stopping comparison after the current audio block…"},
    "status.reference_stopped": {"ko": "비교를 중지했습니다. 마지막 유효 차이를 유지합니다.", "en": "Comparison stopped. The last valid difference remains visible."},
    "status.reference_quiet": {"ko": "현재 입력이 너무 작아 비교를 갱신하지 않았습니다. 마지막 유효 차이를 유지합니다.", "en": "The live input is too quiet to update the comparison. The last valid difference remains visible."},
    "status.reference_failed": {"ko": "레퍼런스 비교 실패 · 기준 데이터와 입력 장치를 확인해 주세요.", "en": "Reference comparison failed · Check the reference data and input device."},
    ...
    "status.cancelling": {"ko": "현재 처리 구간이 끝나는 대로 분석을 취소합니다…", "en": "Cancelling after the current processing block finishes…"},
    ...
    "error.separator_model": {"ko": "AI 기타 분리 모델을 불러오지 못했습니다. 모델 파일 또는 런타임 오류를 확인해 주세요: {detail}", "en": "Could not load the AI guitar model. Check the model files or runtime error: {detail}"},
    "error.separator_network": {"ko": "AI 모델 다운로드 서버에 연결하지 못했습니다. 인터넷이 연결되어 있어도 서버 접근·방화벽·인증서 문제일 수 있습니다. 다시 시도해 주세요: {detail}", "en": "Could not reach the AI model download server. Server access, firewall, or certificate issues can occur even when internet is connected. Please retry: {detail}"},
    "error.separator_cache": {"ko": "AI 모델 캐시 파일에 접근하지 못했습니다. 저장 공간과 폴더 쓰기 권한을 확인해 주세요: {detail}", "en": "Could not access the AI model cache. Check free disk space and folder permissions: {detail}"},
    "error.separator_memory": {"ko": "AI 모델을 준비할 메모리가 부족합니다. 다른 프로그램을 닫거나 CPU 모드로 다시 시도해 주세요: {detail}", "en": "Not enough memory to prepare the AI model. Close other programs or retry in CPU mode: {detail}"},
    ...
    "progress.overall": {"ko": "전체 단계 {percent:.1f}% · 경과 {elapsed}", "en": "Overall stages {percent:.1f}% · Elapsed {elapsed}"},
    "progress.separator_cache": {"ko": "AI 모델 캐시 확인 중…", "en": "Checking the AI model cache…"},
    "progress.separator_download": {"ko": "모델 다운로드 · {filename} · {downloaded_mb}/{total_mb} MB ({percent}%)", "en": "Model download · {filename} · {downloaded_mb}/{total_mb} MB ({percent}%)"},
    "progress.separator_download_bytes": {"ko": "모델 다운로드 · {filename} · {downloaded_mb} MB 수신", "en": "Model download · {filename} · {downloaded_mb} MB received"},
    "progress.separator_loading": {"ko": "모델 다운로드 확인 완료 · 메모리에 불러오는 중…", "en": "Model download checked · loading into memory…"},
    "progress.separating_segment": {"ko": "기타 분리 · {current}/{total} 조각 · 음원 {completed_seconds}/{total_seconds}초 처리 완료", "en": "Isolating guitar · chunk {current}/{total} · {completed_seconds}/{total_seconds}s of audio processed"},
